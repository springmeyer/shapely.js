...
BaseGeom.prototype.buffer = function(val, resolution) {
    if (val === undefined) throw new Error("first arg to buffer must be a number not null");
...
Point.prototype._init = function(coords) {
    var n = coords.length;
    if (n != 2 && n != 3) { throw new Error("invalid point dimensions: " + n); }
    this.cs = lgeos.GEOSCoordSeq_create(1, n);
    ...
    if (n == 3) {
        lgeos.GEOSCoordSeq_setZ(this.cs, 0, coords[2]);
    }
